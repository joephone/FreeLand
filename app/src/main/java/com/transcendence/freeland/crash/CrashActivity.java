...
/**
 * @Author Joephone on 2022/6/17 0017 下午 2:25
...
//        ImmersionBar.setTitleBar(this, findViewById(R.id.ll_crash_info));
        ...
        Throwable throwable = getSerializable("other");
        if (throwable == null) {
            return;
        }
        ...
        builder.append("\n应用版本：\t").append(AppUtils.getVersionName(this))
                .append("\n版本代码：\t").append(AppUtils.getVersionCode(this));
        ...
        try {
            ...
            if (permissions.contains(PermissionPool.CAMERA)) {
                builder.append("\n相机权限：\t").append(ActivityCompat.checkSelfPermission(this, Manifest.permission.CAMERA)!= PackageManager.PERMISSION_GRANTED ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionPool.RECORD_AUDIO)) {
                builder.append("\n录音权限：\t").append(ActivityCompat.checkSelfPermission(this, PermissionPool.RECORD_AUDIO)!= PackageManager.PERMISSION_GRANTED ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionPool.SYSTEM_ALERT_WINDOW)) {
                builder.append("\n悬浮窗权限：\t").append(ActivityCompat.checkSelfPermission(this, PermissionPool.SYSTEM_ALERT_WINDOW)!= PackageManager.PERMISSION_GRANTED ? "已获得" : "未获得");
            }

            if (permissions.contains(PermissionPool.REQUEST_INSTALL_PACKAGES)) {
                builder.append("\n安装包权限：\t").append(ActivityCompat.checkSelfPermission(this, PermissionPool.REQUEST_INSTALL_PACKAGES)!= PackageManager.PERMISSION_GRANTED ? "已获得" : "未获得");
            }
